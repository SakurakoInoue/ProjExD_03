...
class Bird:
    ...
    def update(self, key_lst: list[bool], screen: pg.Surface):
        ...
        # 変更: 移動量の合計が[10, 8]でない場合、self.direを更新
        if sum_mv != [10, 8]:
...
class Beam:
    def __init__(self, bird: Bird):
        ...
        self.vx, self.vy = +5, 0
        # 追加: Birdの方向に合わせてビームを初期化
        radian = math.atan2(-bird.dire[1], bird.dire[0])
        angle_degrees = math.degrees(radian)
        rotated_img = pg.transform.rotozoom(self.img, angle_degrees, 1.0)
        self.img = rotated_img

        # 変更: こうかとんの向きに合わせた初期配置
        self.rct.left = bird.rct.centerx + bird.rct.width * bird.dire[0]
        self.rct.centery = bird.rct.centery + bird.rct.height * bird.dire[1]
